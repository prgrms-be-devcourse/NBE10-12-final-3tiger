...
| 3 | 장소 검색 Rate Limit의 Redis 의존 제거 | 해결 | 단일 서버에서 로컬 Bucket4j로 네트워크 비용과 장애 의존성 제거 |
...
## 10. 3순위 해결: 로컬 Bucket4j Rate Limit

기존에는 Redis Lua 스크립트로 고정 윈도우 Rate Limit을 구현했다. 서버가 한 대인 현재 구성에서는 여러 인스턴스가 카운터를 공유할 필요가 없으므로 모든 장소 검색 요청이 Redis 네트워크 호출에 의존하는 구조를 제거했다.

현재는 사용자 또는 IP별 Bucket4j token bucket을 만들고 Caffeine에 최대 10만 개까지 보관한다. 사용되지 않은 버킷은 설정된 window의 두 배가 지나면 제거된다.
...
최초 버킷 용량       → 30개
요청 1회             → 토큰 1개 소비
토큰 충전            → 60초 동안 30개를 점진적으로 충전
토큰 없음            → PLACE_429_1 응답
```

고정 윈도우는 만료 경계 전후에 허용량의 두 배가 몰릴 수 있지만 token bucket은 시간이 흐르면서 토큰을 점진적으로 복원한다. Bucket4j의 버킷 연산은 스레드 안전하므로 같은 사용자에게 동시 요청이 들어와도 별도 Redis 원자성 처리가 필요 없다.

매 요청의 Redis 왕복과 Redis 장애 전파가 사라지는 대신 애플리케이션 재시작 시 버킷이 초기화된다. 현재처럼 단일 서버에서는 정확한 전체 제한으로 동작하지만, 서버가 여러 대로 확장되면 각 서버에 별도 버킷이 생기므로 Bucket4j Redis 연동 등 분산 저장 방식으로 전환해야 한다.

단위 테스트로 버킷 용량까지 요청 허용, 토큰 소진 후 429 변환 및 사용자/IP별 독립 버킷 생성을 검증했다.
