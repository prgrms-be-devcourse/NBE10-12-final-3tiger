...
| 2 | 길찾기 응답 HTTP 압축 | 해결 | 전체 경로 좌표 전송량 감소 |
| 3 | 외부 HTTP 클라이언트 연결 풀 및 장애 격리 | 미적용 | 대규모 동시 요청 처리와 카카오 장애 전파 방지 |
| 3 | 장소 검색 요청 최적화 및 단기 캐싱 | 미적용 | 동일 검색어에 대한 카카오 API 중복 호출 감소 |
| 3 | Redis Rate Limit 장애 정책 명확화 | 해결 | Redis 장애 시 엄격한 로컬 제한으로 전환해 검색 가용성과 외부 API 보호 유지 |
...
현재 DB의 40개 코스를 확인한 결과 경로 GeoJSON 크기는 중앙값 약 671바이트, 평균 약 1.26KB, 최대 7.07KB였고 좌표 수는 중앙값 25.5개, 평균 49.5개, 최대 284개였다. 현재 데이터 전체가 매우 큰 편은 아니지만 긴 경로는 압축 효과가 충분했다.

| courseId | 비압축 API 응답 | gzip 응답 | 감소율 |
|---:|---:|---:|---:|
| 42 | 7,355B | 2,337B | 68.2% |
| 43 | 4,964B | 1,607B | 67.6% |
| 51 | 4,311B | 1,429B | 66.9% |
| 50 | 3,229B | 1,125B | 65.2% |
| 1 | 2,838B | 1,111B | 60.9% |

42번 응답에서 `Content-Encoding: gzip`과 `Vary: accept-encoding` 헤더를 확인했다. `2KB` 미만의 작은 응답은 압축 CPU 비용을 피하고, 그 이상인 JSON만 압축한다. 로컬 응답 시간은 네트워크 환경을 대표하지 않으므로 운영에서는 서버 CPU 사용량과 모바일 네트워크 응답 시간을 추가로 관찰한다.
...
## 10. 3순위 해결: Redis Rate Limit 장애 정책

현재 Lua 스크립트를 사용한 고정 윈도우 Rate Limit은 원자적으로 동작하지만 윈도우 경계에서 순간적으로 허용량의 두 배에 가까운 요청이 통과할 수 있다.

Redis 정상 상태에서는 기존 Lua 기반 사용자/IP별 `30회/60초` 분산 제한을 유지한다. Redis 연결 실패, 명령 timeout 등의 `DataAccessException` 또는 null 결과가 발생하면 Caffeine 기반 서버 로컬 제한으로 자동 전환한다.
...
```text
Redis 정상   → 전체 서버가 공유하는 30회/60초 제한
Redis 장애   → 서버별 10회/60초 로컬 제한
fallback 초과 → 기존 PLACE_429_1 응답
Redis 복구   → 다음 요청부터 Redis 제한 자동 사용
```

완전한 fail-open처럼 카카오 API 호출을 무제한 허용하지 않고, 완전한 fail-closed처럼 Redis 장애를 장소 검색 전체 장애로 전파하지 않는 절충안이다. 로컬 카운터는 최대 10만 클라이언트만 저장하고 60초 후 만료해 메모리 사용을 제한한다.

서버가 여러 대일 때 fallback 제한은 인스턴스마다 별도로 계산되므로 정확한 전역 제한은 아니다. 예를 들어 서버 3대라면 장애 중 최대 30회까지 허용될 수 있다. 따라서 fallback은 Redis 장애 중의 임시 보호 수단이며 정상 상태의 분산 제한을 대체하지 않는다.

고정 윈도우 경계의 burst가 실제 문제가 되거나 초·분·일 단위 다중 제한, 회원 등급별 정책, 남은 토큰 정보가 필요해지면 Redis 연동 Bucket4j token bucket 전환을 검토한다.
